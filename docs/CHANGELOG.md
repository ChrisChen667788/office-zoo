...
按它规划会走错路。VERSION_PLAN「当前位置」重写为 **7 Phase 完成度对照**(A/B/D/F ✅、
E/G 🚧、**C 真人玩家加入是唯一整块缺口**);ITERATION_PLAN 头部加进度快照 + 竞品数据
时效提示(2026-04 快照,引用前需联网刷新)。

## v6.120 — 2026-06-26 · 模式卡新手/进阶分级(审计玩法 F13)

5 张模式卡视觉权重相同,新用户不知从哪开始。`ModeSpec` 加 `tier` 分级 chip:经典局标
「👶 新手推荐」(绿)、双公司标「🎓 进阶 · 建议先玩经典局」(橙),标题旁小胶囊,给出推荐路径。

## v6.116–v6.119 — 2026-06-26 · 裁了么 · Phase E 文化扩展包(第一批 4 场景)

按 ITERATION_PLAN Phase E 交付第一批新场景,每个都是一堂劳动法速通课(法条口径对齐既有条目):

- **v6.116 `pua-quit-trap` · PUA 逼你「主动辞职」**(难度 2):核心考点「主动辞职=零补偿」
  (46 条)+ 被迫解除(38 条)+ PIP 程序(40 条)。识破辞职信陷阱,拿 N+1 或留任。
- **v6.117 `age-35-optimize` · 35 岁「结构性优化」**(难度 2):无固定期限合同(14 条,
  连续 10 年/两次续签)+ 经济性裁员法定程序(41 条)+ 违法解除 2N(87 条)。「年轻化」不是法定事由。
- **v6.118 `non-compete-trap` · 离职被塞竞业协议**(难度 3):适用人群限定(23/24 条)+
  按月 30% 补偿(司法解释,"已含工资"无效)+ 范围/期限限缩 + 3 个月不付即可解除。
- **v6.119 `overtime-clawback` · 离职清算 996 加班费**(难度 2):150%/200%/300%(劳动法 44 条)+
  「自愿加班承诺书」不免法定义务 + 考勤举证责任倒置 + 存续期间不受一年时效限制。
- 全部走既有 `FiredScenario` 结构,自动进裁了么场景广场;每场自带 hrOpeningLine 陷阱 +
  playerContext 证据牌 + winCondition 胜利线。
...
新用户发现率极低。现在 `grudgeRedirect` 真触发时:engine 发实时 `grudge_vote` 事件 →
event log 一行(🗡️ 橙红背刺风)+ **恩怨录按钮点红**(红点 + 红边 + 发光),打开即视为已读。

## v6.110 — 2026-06-26 · 内部邮件专属动线(审计玩法 F6)

花 200 筹码买「🔍 内部邮件」,结果原本只躺 event log 一行小字 —— 付费体验毫无仪式感。
现在 engine 发专属 `intervene_clue` 事件,全房弹**「内部邮件 · 背景调查」琥珀浮卡**
(顶部居中 4.5s,可点击关闭):「XX 更像资本家那边的人 · 小道消息别全信」。

## v6.109 — 2026-06-26 · 模式卡真实时长预期(审计玩法 F3)

双公司注释写 ~15 分钟、实际 20+;经典局无上限用户更没概念。五张模式卡 features 各补一枚
「⏱ 时长」chip(经典/沉浸 15-25 分钟、裁了么 5-10 分钟/关、段子 1-2 分钟/段、双公司约 20 分钟),
进局前知道要蹲多久。

## v6.108 — 2026-06-26 · Event log 行距/类型边条对比度(审计视觉 F-14)

条目 `marginBottom 2→4` 给呼吸感;类型左边条透明度 `30→55`,高亮度屏(手机)上也分得清
事件类型颜色。

## v6.107 — 2026-06-26 · Landing 赛季 banner 常青化(审计视觉 F-10)

hero 顶部 badge 写死「v6.1 · NEW EVENT」,版本早已跑到 v6.10x,看着像产品停更。
改常青文案「本赛季 · AI 同事会记住你」,不再随版本腐烂。
...
- **F12 · 地图居中**:内容实际横跨 x:135–855(TILE=52/30° 推导),旧 `ORIGIN_X=450` 左空 135px、
  右仅 45px,地图明显偏右;改 405 两侧各留 ~90px。`ORIGIN_Y 60→80` 给顶行头顶的 emote 气泡让出
  空间(旧值下 sy−50 的气泡常被 canvas 顶边裁掉),底部最深 ~496 仍在 550 内不裁。
- **F13 · 房间地板色与阵营色解耦**:产品部 `#2fb8ff`(与猫队光圈同族蓝,猫队站上去混一片)→
  低饱和深蓝 `#1a4a7a`;会议室 `#5e17ff`(近品牌紫/中立玩家紫)→ 深紫 `#2d1a5c`。
- **F11 · badge 拥挤重排**:36px 头像四周原本最多同时挤 4 枚浮动 badge——
  ① 🔥 热门提名从独立浮动 disc **内嵌为名牌前缀**(「🔥名字」+ 橙金描边),左上角让出来;
  ② 双公司 A/B 角标从右下移到**左下**(右上已有 👻 票 badge,对角分布四向不打架);
  ③ **正在发言的鼠不再冒 idle 摸鱼气泡**(发言光圈 + 🥱 同框很分裂,二者互斥)。
- 验证:3 包 tsc 干净;**549 测试绿**;build 通过;preview 真机截图确认地图居中、顶部标签不再
  贴边、产品部/会议室地板色已解耦,零 console error。badge 重排为确定性代码级修改(需真局
  多 badge 同框才能截到,逻辑由 tsc + 参数链把关)。
...
- **F7 · 双公司市占率竞速**:`MARKET_STEP 9→14` + `MARKET_WIN 100→80`。原本每轮 +9~18、到 100 要
  6-11 轮 —— 远超 8 轮上限,竞速赢线基本被人数/内鬼条件抢先终结,招牌「对撞条」前几轮长期死在 0:0。
  现在每轮 +14~28、~3-6 轮可垄断,对撞条随每轮任务真的动,市占赛道才有意义。
- **F8 · 团灭判定**:`dualWinner` 的 `aliveA/B <= 1` 收紧到 `=== 0`。旧逻辑某司剩 1 人就判负,比经典局
  「人数翻盘才结束」严苛得多、旁观者困惑(「还剩 1 人居然就输了?」)。改成**彻底没人才算团灭**;
  市占率/双内鬼皆裁/回合上限仍是更早的正常终局路径。
- **F9 · 抹黑压力**:`SMEAR_PRESSURE 1→2`。1 票在 4-8 人投票里占比太小、只有平局才偶尔有用,机制
  存在感极低;2 票≈一张普通票的分量,真能把舆论压力转成定向开除,搅局意图落地。
- 测试:`dual.test` / `dualEngine.test` 随新阈值/步长更新(改用 `MARKET_STEP`/`MARKET_WIN` 符号而非
  硬编码,防未来调参再回归);团灭用例改为「=0 才判负、剩 1 人继续打」。
- 验证:3 包 tsc 干净;**549 测试绿**;`vite build` 通过。
- ⚠️ **脱口秀活立绘视频(v6.94 待办)再次尝试仍 `insufficient balance`,未生成** —— 需 MiniMax 账号
  充值,客户端已自动回退静态立绘,本版不含视频产物。
...
只教 模式 / 阵营 / 鬼魂功能,**完全没提** v6.74 下注盘🎰 / v6.83 干预商店🛒 / v6.86 双公司🅰🅱 ——
新用户进局看到左下角神秘 🎰🛒 不知是啥、看不懂对撞条,v6.93 刚补完的下注/干预闭环对他们等于隐身。

- **RulesModal 3 → 4 步**:
  - **Step 1** 加「⚔️ 双公司对抗」模式行(两司各 4 鼠各藏 1 内鬼,比市占/团灭/挖角);顺手把
    经典模式 accent 蓝 `#2fb8ff → #4c9eff` 与 v6.93 统一。
  - **新增 Step 4 ·「你的旁观者工具」**:一句话点明旁观者身份(不下场,能下注/干预/爆料),
    + 🎰 下注盘(押谁被开除 / 哪家公司笑到最后,赔率实时动)+ 🛒 干预商店(护盾/线索/聚光灯/猎头)。
- 验证:3 包 tsc 干净;**549 测试绿**;`vite build` 通过;preview 真机 Step 1 四模式(含双公司)
  截图确认;Step 4 全部新内容**确定性编译进 shipped bundle**(headless preview 的 framer-motion
  `AnimatePresence mode="wait"` 在任意翻页都卡内容 —— 预览环境既有现象、非本次回归,真机翻页正常)。
...
(记录在 docs/DESIGN_DUAL_COMPANY.md)。本版只落纯引擎,旧单公司模式零接触。+16 测试 → 492。

- **`shared/dual/dual.ts` 纯引擎**(+16 测试):
  - `assignDualCompanies`:8 鼠 → 4+4,**每公司恰好 1 内鬼**,确定性分配,非法输入退 null;
  - 市占率竞速:`advanceMarket`(任务 ×STEP=9,封顶 100)/`marketWinner`;节奏护栏测试锁死
    「2 任务/轮 → ≤8 轮垄断」对齐 15 分钟拍板;
  - 挖人:`poachChance` = 0.15 + 关系分/200,夹 [5%, 60%](**吃 v6.75 关系网** —— 对面鼠
    跟你有交情就好挖,世仇免谈);`resolvePoach` roll 注入无随机;
  - 跳槽:`DEFECTION_KEEPS_TEAM=true`(拍板②)+ `eventsFromDefection`(老同事全员记
    backstab 喂关系图 —— 跳槽是会被记一辈子的);
  - 跨司动作限额:每公司每回合 1 次(挖人/曝料二选一);
  - `dualWinner` 终局矩阵:🏆 垄断 > 💀 团灭(≤1 人)> 🐀 双内鬼皆裁(比市占率)>
    ⏱ 回合上限兜底;单边内鬼被裁不结束(公司内政)。
- `PlayerState.companyId?: 'a'|'b'`(可选字段,单公司恒 undefined,旧逻辑零影响)。
- 下一刀(v6.85 P2):GameEngine 接线 —— `mode:'dual'` 开关、夜杀限同司、投票按公司分组、
  任务完成喂 `advanceMarket`、AI 跨司动作决策。演出层 v6.86、下注/道具闭环 v6.87。
...
**4 个伴随页 header 没带父页的 AI 图标**。本版抹平 + 360px 全扫兜底。

- **4 个伴随页 header 补 AI 图标**:占卜牌库 / 占卜历史(`nav_fortune`)、我的周报偏好
  (`nav_weekly`)、段子投稿(`nav_talkshow`)—— EventPill 里的裸 emoji 换成 `<Icon>`(emoji 兜底),
  跟各自父页 header 同款。
- **360px 极窄屏全扫**(7 页:三大二级页 + 4 伴随页):Playwright 逐页断言**无横向溢出**
  (scrollWidth=clientWidth=360)+ header 图标真加载(naturalWidth>0)+ 零 pageerror,全绿。
- 诚实记录:原计划的「emoji→图标大替换 + 响应式补课」经 390px 实测**不成立**——三大页早已
  移动优先 + header 带图标;剩余 emoji 是有语义的内容(塔罗牌面/风格标签),保留。
...
- **① 关系网反哺投票权重**(纯函数 `resolveVoteWithGrudge` + 4 测试):基础票之上,候选里若有
  voter 的**真·世仇**(≤ `FOE_VOTE_THRESHOLD` = -60,得跨好几局攒背叛才到)且没投他,就把票
  **真的拽过去** + 甩旧账;普通记仇(-25…-60)不强改,只进 prompt 影响 LLM——有界,世仇稀有,
  不会把社交推理玩成纯复仇。接进 `GameEngine.runVoting`(archetype↔playerId 映射,全程 fail-safe,
  读图失败退回原票),改票时时间线落一条「🗡️ 翻旧账改投」。
- **② 恩怨录进战绩卡 / 分享**:赛后 `Result` 新增「🕸️ 本局恩怨录」——拉 `/api/relations`,只亮
  **本局在场两只鼠之间**的恩怨(红=记仇 / 绿=记恩,带关系档 + 分数);复制回放链接时把头牌恩怨
  捎进分享文案(「办公室恩怨录 Tony💢Lisa(世仇)」)。
- 验证:430 测试全绿;`resolveVoteWithGrudge` 4 单测;throwaway 脚本跑通真 store 往返(两局 backstab
  攒出 -90 世仇 → 基础票被改投世仇 + 甩「又是你?见你一次投你一次」);Playwright 探针确认战绩卡
  恩怨录两行(世仇红 -78 / 过命交情绿 +64)。
- 注:这层结构化社交图谱跟 DB-gated 的 pgvector 情景记忆流互补——那层喂 prompt,这层能画图、
  能反哺投票、能录战绩卡,且不依赖任何外部 DB。
...
这轮补真缺口:**名场面识别太粗**——只挑得出击杀/投票/暴论/终局。+8 测试 → 394。

- **4 种高戏剧性名场面**(`highlightPicker`,纯函数 + 8 测试):
  - `reversal` 反转 —— 接 `predictionLog`,你押 X 结果出局是 Y(声明已久从未产出,这轮真接上)
  - `perfect_bluff` 完美伪装 —— 资本家卧底活到最后没被票出 + 阵营赢
  - `comeback` 绝地翻盘 —— 赢家阵营折损 ≥2 且 ≥ 对家还赢
  - `bloodbath` 腥风血雨 —— 同一回合多人出局
  - 基础分故意压过普通击杀(82/76/70/68 vs 65/60),让"炸点"冒头进 top-3。
- **接线**:`ShareVideoButton` 把 `predictionLog` 喂进挑选器;`videoExport` 帧标签 + server
  `highlightCaption` 都补上新 kind 的文案(影帝时刻/绝地翻盘/腥风血雨),不再错标成"终局"。
- **分享带话题**:系统分享文案按胜负出钩子 + 挂 `#办公室幸存者 #我的AI同事 #OFFICEZOO #班味`,
  便于抖音/小红书聚合。

---

## v6.72 — 2026-06-09 · 打磨:立绘去重 + 群众吐槽降噪

两处去冗余,演出更干净。测试 386 不变。

- **立绘去重**(`EliminationReveal` + `Icon`):AI 表情立绘加载到了,角落 😵/😭 贴纸就**隐藏**
  (AI 图本身已有表情,贴纸冗余);只有立绘没加载到(emoji 兜底没表情)时才补贴纸。`Icon` 新增
  `onResolved(loaded)` 回调,组件据此 tri-state(pending/loaded/failed)决定显隐。
- **群众吐槽降噪**(`ReactionDanmaku` + 经典/沉浸局):以前每次裁员是「静态 2 弹幕 + LLM 1 弹幕
  + 2 条日志」叠加;现在改成**每次只有 1 条**,LLM 那句到了就**替换**静态(弹幕按 `groupId` 清旧
  补新 + 淡出过渡,日志按受控 id 原地替换),不叠加。LLM 失败则静态留着。

## v6.71 — 2026-06-09 · 打磨:表情立绘烤进导出的战报 PNG

补上 v6.70 留的尾巴 —— 复盘屏幕上有立绘,但**导出的分享 PNG**还是 emoji。这下也烤进去了。

- **share card 复盘行画立绘**(`utils/shareCard.ts`):导出前**尽力预加载**复盘前 5 行的 AI 表情
  立绘(惊恐/委屈),`drawImage` 圆角裁切进每行;加载用 `loadImageSafe`(跨域匿名 + 2.5s 超时),
  失败/超时就回退 emoji,导出永不卡死、永不污染 canvas。每行布局右移给立绘让位。
- 验证:经 dev 模块图直接 `generateShareCard` 出图,`toBlob` 成功(CORS 不污染),立绘已栅格进
  PNG,自包含可分享。

## v6.70 — 2026-06-09 · 吐槽念出来 + 立绘进复盘 + 立绘出场音效

裁员演出再补三处:声音 + 复盘 + 出场。测试 386 不变(纯展示/音频层)。

- **群众吐槽接 TTS 念出来**(经典局):LLM 那句拉到后,用浏览器 `speechSynthesis` 以"吃瓜路人
  音"(略快略高)念出来,静音(`isSfxMuted`)时不念。**只在经典局开**——沉浸局的 AI 鼠本就用浏览器
  TTS 配音,群众再插嘴会打断鼠说话,故沉浸局只保留弹幕不念。
- **立绘进 HighlightReel 复盘**:终局战报的「复盘」时间线里,每个被裁的鼠前面补一张 AI 表情立绘
  (惊恐/委屈缩略图,emoji 兜底),一眼认出谁先下岗。导出的分享 PNG 仍走 emoji 自包含,便携不变。
- **立绘出场音效**(`sfx.playReveal`):立绘弹出的瞬间(~150ms)叠一声"啵"——kill→惊恐上扬"啊!"
  + 气声,vote→委屈下行小号呜咽(迷你 sad-trombone),音量压低只当点缀,不抢 kill/vote 主音。

## v6.69 — 2026-06-09 · 立绘接 AI 表情图 + LLM 实时吐槽 + 弹幕从工位飘

把裁员演出从"emoji + 静态池"升级成"AI 立绘 + LLM 即兴 + 有指向性的弹幕"。+6 → 386。

- **立绘接真 AI 表情图**(`iconGen` + `EliminationReveal`):新增 6 张 doubao 生成的二次元表情
  立绘 —— **惊恐 ×3(kill)/ 委屈 ×3(vote)**,按鼠名稳定选一张,填进立绘底盘(圆角裁切),
  没生成时 `<Icon>` 自动回退 emoji 大头。`npm run icons:regen -- expr_*` 本地生成(icons 目录 gitignore)。
- **吐槽接 LLM 实时生成**(`reactionFlavor.ts` + `POST /api/reaction/line`):被裁瞬间把当局上下文
  (谁、什么身份、什么性格)喂给 FIRED_HR_MODEL 链路,生成一句**结合剧情**的群众吐槽(如
  "数据分析师都被投票,难道我们都成了'数据'的一部分?");超时/失败回退 shared 静态池。
  即时先飘静态那条,LLM 那句拉到再追加,体验不卡。纯 prompt/收口/兜底 6 测试。
- **弹幕从被裁工位飘出**(`GameMap.onPositions` + `ReactionDanmaku.origin`):GameMap 在 RAF 里
  节流上报每只鼠的归一化屏幕坐标(写 ref 不触发重渲染),经典局的弹幕就从那只鼠的工位**冒出来 +
  上飘**,而不是随机车道(沉浸局圆桌布局仍横向飘)。

## v6.68 — 2026-06-09 · 群众吐槽弹幕飘地图 + 沉浸局演出对齐

把 v6.67 的群众吐槽从右栏 EVENT LOG 搬上**主战场**,两端演出拉平。测试 380 不变(纯展示层,
复用已测的 reactions 池)。

- **弹幕飘地图**(`components/game/ReactionDanmaku.tsx`):谁被裁 / 被投票出局,就从右往左
  飘一串**吃瓜群众表情包弹幕**(每次 2 条、分车道、错时延、粉色 pill),`pointer-events-none`
  不挡操作,几秒自动清。经典局盖在 `.game-stage`(只飘地图)、沉浸局 `fixed` 飘全屏。
- **沉浸局对齐**:沉浸局也接上群众吐槽弹幕;立绘头像 v6.41 起就已传入,这下两端演出一致
  (立绘弹出 + 群众弹幕 + EVENT LOG 吐槽)。

## v6.67 — 2026-06-09 · 经典局演出强化(被优化立绘弹出 + 群众表情包吐槽)

把"有人被优化"的高光时刻做足节目效果。+7 → 380。

- **被优化角色「表情立绘」弹出**(`EliminationReveal`):裁员/投票出局的大字报上,新增一张
  会**弹跳 + 抖动**的角色立绘 —— 角色大头(优先公司包头像 → 角色绰号 emoji → 事件图标兜底)
  + 左上角刀/票角标 + 右下角表情贴纸(😵/😭)+ 斜盖「已优化 / 已出局」红印章。经典局 vote/kill
  现在也把头像传给立绘。
- **聊天框群众表情包吐槽**(`data/reactions.ts` 纯池 + 7 测试):EVENT LOG 里,谁被优化 / 被投票
  出局 / 被爆料,紧跟一条**吃瓜群众**的阴阳怪气弹幕(`🤡 群众:又一个 KPI 战士光荣下岗`),
  粉色区别于正经播报。按事件 id 取词(确定性,不抖动)。

## v6.66 — 2026-06-08 · 战绩榜 + 第 4 档 BOSS + 牌局反哺主对局

闯关牌局三件套:战绩进全网榜、加一档满级 BOSS、把牌局接回主对局。+17 → 373。

- **全网战绩榜**(P1):打完自动上报本局(遣散费 / 赔偿档 / 对手 / 职级 / 结局),服务端
  按 userId 留**个人最佳**一条。纯排序层 `negotiation/leaderboard.ts`(`runScore` 遣散费
  主导 → 赔偿档 → 利落度 · `upsertBestRun` · `rankRuns`)+ server `negRunStore.ts` +
  `POST /api/negotiation/score` / `GET /leaderboard`(限流 60/h,userId 截断 8 位)。准备界面加
  「🏆 全网战绩榜」面板 —— 每行可 **📸 重画那局的战绩卡**(晒图入口,复用 v6.65 卡面)。
- **第 4 档 BOSS「🐉 资本本尊」**(P2):满级「劳动法之神」专属,预算 170 / 耐心 6 /
  赔率 ×3.6 —— 出钱的人亲自来,要么 N+6,要么仲裁见。`BOSS_TIERS` 第 4 档,职级门槛 Lv.4。
- **牌局反哺主对局**(P3):主对局(经典 / 沉浸)任一只鼠被裁,旁观席每行加「⚔️ 谈赔偿」——
  跳进闯关牌局**替 TA 谈**。纯桥接 `negotiation/bridge.ts`(按鼠名稳定挑 BOSS + 职级夹取 +
  横幅文案);牌局准备界面读 `?for=` 显示「替「X」找 {BOSS} 谈赔偿」横幅并预选对手。

## v6.65 — 2026-06-08 · 赔偿结算战绩卡(打完一键生成 / 分享 PNG)

闯关玩法线收口:每局打完出一张 1080×1350 竖图战绩卡,一键系统分享 / 下载。+7 → 356。

- **纯文案层**(`negotiation/shareCard.ts`):`buildSettlementCard` 把一局结果摊成卡面
  内容(标题 / 基调 / 数据行 / 一句嘴替),`settlementVibe` 定基调 —— 2N/3N=赢(绿)、
  N+1/认怂还有=平(黄)、掀桌/空手=输(红)。掀桌标题点名「HR 掀桌走仲裁」,认怂 / 没谈成
  各有专属文案。纯函数,7 测试锁死(标题 / 行 / 遗物可选行 / 基调)。
- **canvas 绘制**(client `utils/negotiationShareCard.ts`):深底 + vibe 色径向光晕 + 赔偿档
  巨字(白→vibe 渐变)+ 数据行卡(对手 / 赔偿档 / 用时 / 遗物 / 收获 / 职级)+ 嘴替 +
  品牌脚注;`shareNegotiationCard`(移动端系统分享,不支持自动回退)+ `downloadNegotiationShareCard`。
- **接入**:闯关结算弹层加「📸 生成战绩卡」按钮,用当局 outcome / boss / 遗物 / 奖励 / 职级
  组卡。Playwright 经 dev 模块图直接渲染校验卡面。
...
零新生成:**裁了么 ⚖️ / Premium 👑 / 深夜酒馆 🍺 / 打工人测试 🧠 / 攒局 🎭 / 周年 🎉 /
我的 🪪**,全部走 `<Icon>` 带 emoji 兜底(本地无图自动回退,不崩)。至此首页二级入口 +
主要二/三级界面(占卜/单口/周报/裁了么/Premium/酒馆/测试/攒局/周年/我的)的 headline
图标都是 AI 二次元图。牌局 UI 的预算/耐心/筹码/话术卡小图标留作后续。
