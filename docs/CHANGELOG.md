...
## v6.90 — 2026-06-14 · 健壮性:玩家列表去重,堵住整端白屏崩溃

真机跑双公司局时抓到:某次 `game:state` 发来含**重复 id** 的玩家列表(同一 socket 串了
两局的状态),BettingBar 回合盘 / KillFlashOverlay 的 `AnimatePresence` 因重复 React key
**直接抛错 → 整个旁观端白屏卸载**(root 清空,无 error boundary 兜底)。

- **`gameStore.updateState`**:写入前按 id 去重玩家(取首次出现)。一处兜住所有下游
  重复-key 隐患(回合盘 / 地图 / 裁员闪屏 / 立绘),无论服务端发来什么都不再崩。
- 验证:真机复现崩溃 → 加去重 → 同样路径不再白屏;client tsc 干净。
- 注:重复状态的根因(单 socket 串两局)是手测时连开两局所致,正常单局不触发;去重是
  防御性硬化,成本一行。
