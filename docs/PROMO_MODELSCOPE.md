...
> **2026-05-27 更新 v6.34**: 同步 README 飞书化结构 + 文案去攻击性 + 微信小程序端上线。
...
| **MVP 周期** | 34 轮迭代 (v6.0 → v6.34), 每轮 1-7 个 P, 104 个 vitest 全绿 |
| **可玩端** | Web (PC + 移动) · 微信小程序 · ModelScope 创空间 |
...
| **没人能讲** | 深夜酒馆 1v1 跟某只 AI 鼠人喝酒互相吐槽班味, lo-fi BGM |
| **想笑着学劳动法** | 截了么 5 关闯关, 每关绑《劳动合同法》一条, 可玩可学 |
| **想看大模型在演什么** | 全程透明 prompt 设计 · 34+ 轮迭代日志 · mermaid 架构图 + 时序图开源 |
...
## 🆕 v6.12x 双公司商战 Hook(当前主推,2026-07 刷新)

> **两家 AI 公司同台互卷,观众下注、干预、带节奏 —— 你的每个操作真的改剧情。**
>
> ⚔️ 双公司对抗 + 🎰 观众下注经济 + 🎨 整站米哈游化 + 🖼️ 27 张二次元立绘。
> 全开源 MIT,docker compose 一键起。

### v6.12x 三句话讲清楚

1. **双公司商战是新王牌** — 4v4 两家公司各藏 1 个资本家内鬼:市占率竞速「对撞条」、
   跨司挖角跳槽(内鬼身份带走!)、商业抹黑曝料(真假参半的舆论战),终局矩阵
   垄断 > 团灭 > 双鬼皆裁 > 回合上限 —— 每局都是一部商战剧。
2. **观众不再只是看客** — 筹码押「谁被开除」(鬼魂票 + 你的「🤥 在演」判断实时改赔率),
   赢的筹码买干预道具:🛡 护盾挡刀 / 🔍 内部邮件曝阵营 / 🎭 聚光灯加戏 / 📦 猎头快递逼跳槽,
   每一件都真进引擎改剧情;被拒还退筹码给真实原因。
3. **Z 世代审美全面换皮** — 对局/段子/裁了么全线 gradient mesh + 相位元素色
   (冷蓝待机→红全员会→粉撕逼→金投票)+ Landing 六边形 stigma 卡;24 张职场 archetype
   立绘 + 3 档黑心 HR 反派立绘 + 00 后「整顿王」persona;裁了么新增 PUA/35 岁/竞业/加班费
   四关劳动法速通课。
...
> "周报是每个打工人的灾难" — 这是 v6.5 的 viral 引擎, 优于"AI 玩游戏"
> 的抽象介绍, 转化率高 1 个量级。

**Hook 3 行:**

> **AI 周报生成器: 1 句关键事件 → 4 种风格周报。**
>
> 🧩 阿里黑话版 / 🎭 PUA 版 / 🎩 装腔版 / 💢 直球版
> 4 卡拼成一张 1080×1350 PNG, 一键发圈, 让朋友选哪个版本他老板最爱听。

### v6.5 三句话讲清楚

1. **周报这件事是普世痛点** — 不分国企外企大厂小厂, 每周都得交一份。
   AI 把同一事件用 4 种公司话术包装, 对照感强烈。
2. **不是单调 LLM 包装** — 每种风格都有精心写过的硬性 prompt 规则
   (阿里版必须 ≥6 个黑话, PUA 版必须把客观困难转为主观问题, 装腔版必须
   引用商业大佬名言), 输出质量稳定且有"性格"。
3. **结果支持一键拼 PNG** — 4 卡 2×2 网格 + 米哈游金边 + 顶部事件
   引用, 直接微信 / 小红书 / Twitter 全平台兼容。
...
2. **它会跨局记你的仇** — pgvector + Stanford Smallville 风 reflection 层,
   你坑过哪个 AI, 下一局他第一句话就提你: "上次说我颗粒度不够, 还联合Tyler
   投我出局, 这波闭环操作真是打得漂亮。"
